...
import aiomysql # Changed from mysql.connector
import asyncio
import random
import string
import datetime
    ...
    async with db_pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            try:
                ...
                decay_players = await cursor.fetchall()
            ...
            async with conn.cursor() as cursor:
                try:
                    ...
                    if strike_approved:
                        ...
                        await strike_user(mock_ctx, target_user_obj, strike_reason)
    ...
    async with db_pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            try:
                ...
                ppp_manager_role = get_role_by_name(ctx.guild, PPP_MANAGER_ROLE_NAME)
                is_ppp_manager = ppp_manager_role in ctx.author.roles if ppp_manager_role else False
            ...
            try:
                ...
                if len(pages) > 1:
                    ...
                    def check_reaction(reaction: discord.Reaction, user_react: discord.User):
                        return user_react == ctx.author and str(reaction.emoji) in ["⬅️", "➡️"] and reaction.message.id == message.id
...
async def admin_view_game(ctx: commands.Context, game_no: int):
    ...
    if game_data['status'] == 'completed':
        ...
        if game_data['scored_by'] and str(game_data['scored_by']).isdigit(): # Check if it's a Discord ID
